...
import (
	...
	dtoError "github.com/edgexfoundry/edgex-go/internal/pkg/v2/application/dto/v2dot0/error"
...
func UseCaseRequest(w http.ResponseWriter, r *http.Request, version, kind, action string, router *routing.Router) {
	defer func() {
		if r.Body != nil {
			_ = r.Body.Close()
		}
	...
	// is request for a use case that doesn't have a request DTO defined (i.e. routable.EmptyRequest() returns nil)?
	if request == nil {
...
func statusToHTTPStatusCode(status application.Status) int {
	if status == application.StatusSuccess {
